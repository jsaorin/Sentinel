		...
		const results = await Promise.all(
			multisigsWithActivity.map(async (m) => {
				...
				return props.map((p) => ({
					...
					multisig: m.label ?? `${m.address.slice(0, 4)}..${m.address.slice(-4)}`,
					riskLevel: (p.riskScore != null ? getRiskLevel(p.riskScore) : "unknown") as RiskLevel,
					...
					{wallets.length === 0 ? (
						...
						wallets.map((w, i) => (
							<StatsListRow
								...
								metadata={w.threshold != null ? `Squads ${w.threshold}/${w.totalSigners}` : "Squads"}
								riskLevel={w.healthScore != null ? getLevel(w.healthScore) : "unknown"}
